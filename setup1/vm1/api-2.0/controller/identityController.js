...
export const registerIdentity = async (req, res, next) => {
    ...
    logger.debug("Flag5.1")
    const adminUser = await provider.getUserContext(adminIdentity, 'admin');
    let secret, x509Identity, enrollment;
    try {
        logger.debug("Flag5")
        // Register the user, enroll the user, and import the new identity into the wallet.
        secret = await ca.register({ affiliation: orgname == 'Org1' ? 'org1.department1' : 'org2.department1', enrollmentID: username, role: 'client' }, adminUser);
        logger.debug("Flag6")
