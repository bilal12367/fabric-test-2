...
float=$(echo "$version" | awk '{print $0 + 0}')

echo $float

VERSION=$(echo "$float" | bc)
...
chaincodeQuery() {
    ...
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"function": "QueryAllProduct","Args":[]}'
